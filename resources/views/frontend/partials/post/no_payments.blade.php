            ...
            <div class="media-body">
                <div>
                    ...
                    @if($post->user->role == 'Admin')
                        <span class="mod" data-bs-toggle="tooltip" aria-label="Admin" data-bs-original-title="Admin">
                            <i class="bi bi-octagon-half"></i>
                        </span>
                    @endif

                    @if($post->user->role == 'Moderator')
                        <span class="mod" data-bs-toggle="tooltip" aria-label="Moderator" data-bs-original-title="Moderator">
                            <i class="bi bi-hexagon-half"></i>
                        </span>
                    @endif

                    @if ($post->user->user_sub($post->user->id)->plan_id != 1)
            ...
            @if (Auth::check())
                @if (Auth::user()->id === $post->user->id)
                    ...
                    <div class="dropdown-menu dropdown-menu-end">
                        ...
                        <a class="dropdown-item" href="javascript:void(0)" onclick="delete_post('{{ route('user.posts.destroy') }}','{{ $post->id }}','{{ trans('delete_this_post') }}');"><i class="bi bi-trash3 me-2"></i> {{ trans('delete') }} {{ trans('post') }}</a>
                    </div>
                @endif
            @endif
        </div>
                            ...
                            <div class="media align-items-center">
                                ...
                                <div class="media-body">
                                    <div><a href="{{ route('user', ['username' => $comment->user->username]) }}">{{ $comment->user->name }}</a>
                                        ...
                                        @if($comment->user->role == 'Moderator')
                                            <span class="mod" data-bs-toggle="tooltip" aria-label="Moderator" data-bs-original-title="Moderator">
                                                <i class="bi bi-hexagon-half"></i>
                                            </span>
                                        @endif

                                        @if ($comment->user->user_sub($comment->user->id)->plan_id != 1)
                                            <span class="mod" data-bs-toggle="tooltip" aria-label="Pro" data-bs-original-title="Pro">
                                                <i class="bi bi-star-half"></i>
                                            </span>
                                        @endif
                                        ...
                                        <div class="media align-items-center">
                                            ...
                                            <div class="media-body">
                                                <div><a href="{{ route('user', ['username' => $reply->user->username]) }}">{{ $reply->user->name }}</a>
                                                    ...
                                                    @if($reply->user->role == 'Moderator')
                                                        <span class="mod" data-bs-toggle="tooltip" aria-label="Moderator" data-bs-original-title="Moderator">
                                                            <i class="bi bi-hexagon-half"></i>
                                                        </span>
                                                    @endif

                                                    @if ($reply->user->user_sub($reply->user->id)->plan_id != 1)
                                                        <span class="mod" data-bs-toggle="tooltip" aria-label="Pro" data-bs-original-title="Pro">
                                                            <i class="bi bi-star-half"></i>
                                                        </span>
                                                    @endif
